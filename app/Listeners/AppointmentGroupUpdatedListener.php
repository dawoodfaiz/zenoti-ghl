...
class AppointmentGroupUpdatedListener implements ListenerInterface
{
    ...
    {
        $servicesToBeExcluded = ['Neurotoxin Follow up', 'Consult Follow-Up', 'Dermal Filler Follow Up', 'Touch Up Botox', 'LHR TouchUp 30 minutes', 'Good Faith Consultation', 'Consult', 'Tattoo Consult', 'LHR Consultation', 'Consult for injectables', 'Phone Consult for Good Faith', 'CoolSculpting Consultation', 'Good Faith Consult(2)'];
        ...
        $appointmentPostDataGHL = [
            ...
            'calendarId' => $goHighLevelService->getCalendarID(),
        ...
        $appointmentIDGHL = $this->dbService->query("SELECT ghl_appointment_id FROM {$this->dbService->tablePrefix}_appointments WHERE zenoti_appointment_id = :appointment_id ORDER BY id DESC LIMIT 1", ['appointment_id' => $appointmentID])->fetchColumn();

        if ($appointmentIDGHL) {
            $appointmentResponseGHLJSON = $goHighLevelService->updateAppointment($appointmentIDGHL, $appointmentPostDataGHL);
        } else {
